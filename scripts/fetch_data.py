...
TOP_N     = 30          # number of top USDT pairs to pre-generate
TIMEFRAMES = ['15m', '4H']
LIMIT      = 300
...
# ── 2. Top USDT pairs by 24h volume ─────────────────────────────────────────
...
print(f'  Top pairs: {", ".join(top_pairs[:5])} …')

# ── 3. Candles for each top pair ────────────────────────────────────────────
available = {}
errors    = []
...
for inst in top_pairs:
    for bar in TIMEFRAMES:
        try:
            ...
            time.sleep(0.12)   # stay well under OKX rate limit
        except Exception as e:
            errors.append(f'{inst}/{bar}: {e}')

if errors:
    print(f'  Errors: {errors}')
print(f'  ✓ Candles saved for {len(available)} pairs × {TIMEFRAMES}.')
